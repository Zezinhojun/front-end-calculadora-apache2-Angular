...
export class SheetsService implements ISheetService {
      ...
      .pipe(
        ...
        map((response: ISheetsResponse) => {
          if (Array.isArray(response.values)) {
            return response.values.map(PacienteMapper.fromSheetsResponse)
  ...
  deleteRow(rowIndex: number): Observable<boolean> {
    return this._http.post<boolean>(`${ApiGoogleSheetsEndpoint.GoogleSheets.DeleteRow}`, { lineId: rowIndex })
  }
