...
export default class PacienteFormComponent implements OnInit {
  ...
  public onSubmit() {
    if (this.form.valid) {
      ...
      if (glimControl && internacaoControl) {
        ...
        const formattedDateInternacao = this.formatDate(
          internacaoControl.value,
        );
  ...
  public filter(): void {
    ...
    this.filteredOptions = this.patologies.filter((o) =>
  ...
  private getTreatments(): void {
    this._sheetSvc.getTreatments().subscribe({
      next: (data) => {
        this.atendimentoList = data.values.map((item: any[]) =>
          Number(item[0]),
        );
      },
    });
